# Supplementary. Sensitivity analysis for the rVSV application
# (analysis/application/RISE-rVSV.R): effect of fixing the
...
# composite. Data splitting (PREVAC screening, Hamburg evaluation) is
...
table_path <- fs::path(output_path, "tables", "supplementary", "epsilon_sensitivity_rvsv.tex")
...
build_epsilon_sensitivity_table(
  ...
  caption = "Sensitivity analysis evaluating the effect of varying the screening-stage non-inferiority margin epsilon on the rVSV/placebo (PREVAC, screening) surrogate signature, evaluated in rVSV (Hamburg).",
  label = "tab:epsilon-sensitivity-rvsv"
)
