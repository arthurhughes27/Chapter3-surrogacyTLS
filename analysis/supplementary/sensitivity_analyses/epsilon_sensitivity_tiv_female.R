# Supplementary. Sensitivity analysis for the SDY1276 (TIV, Female)
# application (analysis/application/RISE-SDY1276.R): effect of fixing
# the screening-stage non-inferiority margin epsilon at several values
# (instead of choosing it adaptively) on the resulting gamma_Gamma
# composite. Data loading and the train/test sample split are identical
# to the main application; only the screening-stage epsilon is varied.
...
table_path <- fs::path(output_path, "tables", "supplementary", "epsilon_sensitivity_tiv_female.tex")
...
saveRDS(sensitivity_df, fs::path(results_path, "epsilon_sensitivity_tiv_female.rds"))

build_epsilon_sensitivity_table(
  sensitivity_df, out_path = table_path,
  caption = "Sensitivity analysis evaluating the effect of varying the screening-stage non-inferiority margin epsilon on the SDY1276 (TIV, female) surrogate signature.",
  label = "tab:epsilon-sensitivity-tiv-female"
)
