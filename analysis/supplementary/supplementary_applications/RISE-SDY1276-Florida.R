...
# train/test), analysing the B/Florida/4/2006 strain-specific antibody
# response instead of the main application's mean-across-strains
# response (ab_p_28/ab_p_0) - otherwise identical to
# analysis/application/RISE-SDY1276.R (same cohort: sex == "Female").
...
yone_col <- find_strain_column(colnames(df_merged_all), 28, "florida")
yzero_col <- find_strain_column(colnames(df_merged_all), 0, "florida")
...
rise_res <- run_rise_pipeline(
  ...
  figure_path = fs::path(figure_path, "rise_sdy1276_florida_screening_evaluation.pdf"),
...
saveRDS(markers, fs::path(results_path, "TLS_TIV_SDY1276_Florida.rds"))
...
saveRDS(
  ...
  fs::path(results_path, "screening_sdy1276_florida.rds")
)
...
writeLines(as.character(genelist), con = fs::path(results_path, "genelist_SDY1276_Florida_DAVID.txt"))
writeLines(as.character(backgroundlist), con = fs::path(results_path, "backgroundlist_SDY1276_Florida_DAVID.txt"))
