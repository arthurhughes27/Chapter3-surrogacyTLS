...
# train/test), analysing the A/Brisbane/59 strain-specific antibody
...
yone_col <- find_strain_column(colnames(df_merged_all), 28, "brisbane_59")
yzero_col <- find_strain_column(colnames(df_merged_all), 0, "brisbane_59")
...
saveRDS(markers, fs::path(results_path, "TLS_TIV_SDY1276_Brisbane59.rds"))
...
saveRDS(
  ...
  fs::path(results_path, "screening_sdy1276_brisbane59.rds")
)
...
writeLines(as.character(genelist), con = fs::path(results_path, "genelist_SDY1276_Brisbane59_DAVID.txt"))
writeLines(as.character(backgroundlist), con = fs::path(results_path, "backgroundlist_SDY1276_Brisbane59_DAVID.txt"))
